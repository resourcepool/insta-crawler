...
const IgersElementsFinder = class {
	...
	async getQuerySelectors() {
		...
		let profile = {
			...
			// Find post URL
			...await this._findPostUrl()
		};

		// Get first post url of template
		let url = await this.page.$eval(profile.postUrl, elem => elem.href);

		// Get query selector for post-related data with current template

		let posts = await new IgersPostElementsFinder(url, this.browser).getQuerySelectors();
	...
	async _findUsernameAndDisplayName() {
		let names = await this.page.$$eval("h1", async (h1s, conf) => {
			let r = {
				username: null,
				displayName: null
			};
			for (let i in h1s) {
				if (h1s[i].innerHTML === conf.username) {
					r.username = "h1" + await window.computeClassNames(h1s[i].className);
				} else if (h1s[i].innerHTML === conf.displayName) {
					r.displayName = "h1" + await window.computeClassNames(h1s[i].className);
				}
			}
			return r;
		}, template);
		if (!names || !names.username || !names.displayName) {
			console.error(names);
	...
	async _findPostUrl() {
		let postUrl = await this.page.$$eval("img", async (els, conf) => {
			...
			return "div" + await window.computeClassNames(postRootNode.className) + " a";
		}, template);
		if (!postUrl) {
			throw new Error("Couldn't get url of post");
		}
